  ...
  try {
    ...
    for (const item of items) {
      ...
      if (!productId || !userId || !quantity || !name || !email || !address) {
        return res.status(400).json({
          error: 'Missing required fields in reservation item'
        });
      }
...
exports.getAllReservations = async (req, res) => {
  try {
    const reservations = await Reservation.find().populate('productId').populate('userId', 'nom email');
...
exports.getOrdersByVendor = async (req, res) => {
  ...
  try {
    const orders = await Reservation.find({
      vendorId
    }).populate('productId').populate('userId', 'nom email');
    res.json(orders);
  } catch (error) {
